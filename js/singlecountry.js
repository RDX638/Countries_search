...
const updateUi = (_single) => {
  ...
  div.innerHTML = `
        ...
        <div class=" col-md-6 m-2 overflow-hidden ">
            <img class="img-fluid border border-dark img-country width="100% height="100%" src="${
              _single.flags.png
            }" alt="${_single.name.common}" width="100%" >
        </div>
        <div class=" col-md-5 ms-2 text-start headline ">
        <h3>${_single.name.common} </h3>
       <p> Population: <strong> ${(
         Math.floor((_single.population / 1000000) * 100) / 100
       ).toLocaleString(2)}M</strong></p>  
        ...
        marginheight="0" 
        marginwidth="0" 
        src="https://maps.google.com/maps?q=${_single.latlng[0]},${
    _single.latlng[1]
  }&hl=en&z=5&amp;output=embed"
        >
    </iframe>
    <div>
...
`;
  ...
  if (_single.borders) {
    ...
    _single.borders.forEach((item, i) => {
      ...
      if (item != "PSE") {
        border.innerHTML =
          i !== _single.borders.length - 1 ? `${item}, ` : `${item}`;
    ...
    document.querySelector("#id_borders").innerHTML =
      "borders :<strong>None</strong>";
  }
